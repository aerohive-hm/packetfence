...
__PACKAGE__->config(
    ...
    'View::JSON' => {
       ...
       expose_stash    => [ qw(status status_msg filePath error interfaces networks switches config services success items time_offset trial entitlement update_info update_progress CN_Server CN_CA Server_INFO CA_INFO Cert_Content A3_data CA_file_path Server_file_path) ], # defaults to everything
    },
