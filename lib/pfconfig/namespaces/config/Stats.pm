...
sub build_child {
    ...
    foreach my $network (keys $self->{network_config}) {
        ...
        $tmp_cfg{"metric 'total dhcp leases remaining on $network' past day"} = {
            'type' => 'api',
            'statsd_type' => 'gauge',
            'statsd_ns' => 'source.a3.dhcp_leases_'.$network.'_day',
            'api_method' => 'GET',
        ...
        $tmp_cfg{"metric 'total dhcp leases remaining on $network' past week"} = {
            'type' => 'api',
            'statsd_type' => 'gauge',
            'statsd_ns' => 'source.a3.dhcp_leases_'.$network.'_week',
            'api_method' => 'GET',
            'api_path' => "/api/v1/dhcp/stats/$dev/$network",
            'api_compile' => '$[0].free',
            'interval' => '168s',
        ...
        $tmp_cfg{"metric 'total dhcp leases remaining on $network' past month"} = {
            'type' => 'api',
            'statsd_type' => 'gauge',
            'statsd_ns' => 'source.a3.dhcp_leases_'.$network.'_month',
            'api_method' => 'GET',
            'api_path' => "/api/v1/dhcp/stats/$dev/$network",
            'api_compile' => '$[0].free',
            'interval' => '720s',
        ...
        $tmp_cfg{"metric 'total dhcp leases remaining on $network' past year"} = {
            'type' => 'api',
            'statsd_type' => 'gauge',
            'statsd_ns' => 'source.a3.dhcp_leases_'.$network.'_year',
            'api_method' => 'GET',
            'api_path' => "/api/v1/dhcp/stats/$dev/$network",
            'api_compile' => '$[0].free',
            'interval' => '8760s',
