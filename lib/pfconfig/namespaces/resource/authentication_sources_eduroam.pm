package pfconfig::namespaces::resource::authentication_sources_eduroam;

=head1 NAME

pfconfig::namespaces::resource::authentication_sources_eduroam
...
sub build {
    ...
    while ( my ($id, $data) = each %{$self->{_authentication_config}->{authentication_config_hash}}) {
        next unless $data->{'type'} eq "Eduroam";
        $hash{$id} = $data;
    }
    return \%hash;
}
