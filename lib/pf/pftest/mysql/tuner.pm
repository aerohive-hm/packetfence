package pf::pftest::mysql::tuner;

=head1 NAME

pf::pftest::mysql::tuner
...
pf::pftest::mysql::tuner
Incorporated the mysqltuner.pl script into pftest framework
...
sub mysql_setup {
    require pf::config;
    $opt{host} = $pf::config::Config{database}{host};
    $opt{port} = $pf::config::Config{database}{port};
#    $opt{forcemem} = 1877;
...
sub parseArgs {
    my ($self) = @_;
    my $arg_count = $self->args;
    return $arg_count <= 2;
}

sub _run {
    my ($self) = @_;
    my ($user,$pass) = $self->args;
    $opt{user} = $user if defined $user;
    $opt{pass} = $pass if defined $pass;
